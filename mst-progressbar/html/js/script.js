...
$('document').ready(function () {
    ...
    Progressbar.Progress = function (data) {
        ...
        $(".progress-container").fadeIn('fast', function () {
            $("#progress-bar").stop().css({ "width": 0, "background-color": "#08173f" }).animate({
                width: '100%'
            }, {
                ...
                complete: function () {
                    $(".progress-container").fadeOut('fast', function () {
                        ...
                        $.post('http://mst-progressbar/FinishAction', JSON.stringify({
                        })
                        );
                    })
                }
    ...
    Progressbar.ProgressCancel = function () {
        ...
        cancelledTimer = setTimeout(function () {
            $(".progress-container").fadeOut('fast', function () {
                $("#progress-bar").css("width", 0);
                $.post('http://mst-progressbar/CancelAction', JSON.stringify({
                })
                );
            });
        }, 1000);
